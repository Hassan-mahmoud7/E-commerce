                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title"><b>{{ __('dashboard.sliders') }}</b></h4>
                            ...
                            <div class="card-content collapse show">
                                <div class="card-body">
                                    <button class="btn btn-success px-2 round btn-glow" data-toggle="modal"
                                        ...
                                        {{ __('dashboard.create_slider') }}</button>
                                    ...
                                    <div class="card-body card-dashboard">
                                        ...
                                        <table id="yajra_table"
                                            ...
                                            <thead>
                                                <tr>
                                                    <th scope="col">#</th>
                                                    <th scope="col">{{ __('dashboard.slider_image') }}</th>
                                                    <th scope="col">{{ __('dashboard.slider_note') }}</th>
                                                    <th scope="col">{{ __('dashboard.slider_created_at') }}</th>
...
@push('js')
    ...
    <script>
            ...
            responsive: {
                details: {
                    display: DataTable.Responsive.display.modal({
                        header: function(row) {
                            var data = row.data();
                            return 'Details for {{ __('dashboard.slider') }}:' + data['code'];
                        }
                    }),
        ...
        $(document).on('click', '.delete_confirm_btn', function(e) {
            ...
            var delete_slider_id = $(this).attr('slider_id');
            Swal.fire({
                title: title,
                text: text,
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('dashboard.sliders.destroy', ':id') }}".replace(':id',delete_slider_id),
                        method: "DELETE",
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
